	...
	return (
		<>
			...
			<div className='chooseDateComponent'>
				{
					props.timeArr.map((e, ind) => {
						return <span key={ind} onClick={() => props.getTime(e)}>{e}</span>
					})
				}
			</div>
		</>
...
const ChooseDateComponent = ({handler, dateArray}
	...
	const [dataToAdd, setDataToAdd] = useState({data: null, from: null, to: null})
	const [secondDates, setSecondDates] = useState([])
	const clickDate = (date: number) => {
		setHideMonthComponent(true)
		setShowFirstDate(true)
	}
	const hideFirstStep = () => setDaysInMonth((prev) => dateArray.map(el => el.day))
...
//todo add check difference between time
	...
	return (
		<>{
			showSecondDate &&
			<ChooseTimeComponent text="Время пробуждения"
													 timeArr={secondDates}
													 getTime={(data) => {
			...
			/>}
			{
				showFirstDate && <ChooseTimeComponent text="Время отхода ко сну"
																							timeArr={timeArray}
																							getTime={(data) => {
			...
			{!hideMonthComponent && <div className='chooseDateComponent'>
				{
					daysInMonth.map((e, i) => {
						return (
							<span onClick={() => {
