...
const CanvasChart = (props: any) => {
  ...
  const barsData = useSelector(
    (state: stateType) => state.canvasDataChart.data
  );
  ...
  useEffect(() => {
    ...
    setCanvasElement(
      new CanvasComponent(canvasWrapperRef.current, 700, 500, barsData)
    );
  }, [barsData]);

  useEffect(() => {
    !email ? nav("/") : dispatch(getCanvasData({ email }));
  }, []);
